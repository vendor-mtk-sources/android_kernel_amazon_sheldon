...
void mii_mgr_read_combine(struct mtk_eth *eth, u32 phy_addr, u32 phy_register,
			  u32 *read_data)
{
	pr_info("%s phy %d reg 0x%x\n", __func__, phy_addr, phy_register);
...
int mtk_do_priv_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case MTKETH_ESW_REG_READ:
		...
		pr_info("%s esw read 0x%x is 0x%x", __func__, reg.off, reg.val);
	...
	case MTKETH_ESW_REG_WRITE:
		...
		pr_info("%s esw write 0x%x 0x%x", __func__, reg.off, reg.val);
