...
static u8 local_page_buf[NAND_MAX_PAGESIZE];
...
static const snand_flashdev_info gen_snand_FlashTable[] = {
	...
	{{0x2C, 0x34}, 2, 512, 128, 4096, 256, 0x00000000, 0x00000000, 0x1A00001A,
		0x00000000, 0x0552000A, 0x01, "MT29F4G01ABAFD", 0x00000000},
	...
	{{0x6B, 0x01}, 2, 256, 128, 2048, 128, 0x00000000, 0x00000000, 0x1A00001A,
		0x00000000, 0x0552000A, 0x01, "CS11G1T0A0AA", 0x00000000},
	{{0x6B, 0x02}, 2, 512, 128, 2048, 128, 0x00000000, 0x00000000, 0x1A00001A,
		0x00000000, 0x0552000A, 0x01, "CS11G2T0A0AA", 0x00000000},
	{{0x6B, 0x00}, 2, 128, 128, 2048, 128, 0x00000000, 0x00000000, 0x1A00001A,
		0x00000000, 0x0552000A, 0x01, "CS11G0T0A0AA", 0x00000000},
	{{0x6B, 0x10}, 2, 128, 128, 2048, 128, 0x00000000, 0x00000000, 0x1A00001A,
		0x00000000, 0x0552000A, 0x01, "CS11G0G0A0AA", 0x00000000},
};
...
static bool mtk_snand_get_device_info(u8 *id, snand_flashdev_info *devinfo)
{
	...
	if (target != -1) {
		...
		for (n = 0; n < gen_snand_FlashTable[target].id_length; n++) {
			...
			pr_info("%x ", devinfo->id[n]);
		}

		pr_info("], [%s], Page[%d]B, Spare [%d]B Total [%d]MB\n",
...
static bool mtk_snand_reset_dev(struct mtk_snfc *snfc)
{
	...
	for (;;) {
		...
		timeout--;
		if (!timeout) {
			pr_info("SNAND_STATUS_OIP timeout!!\n");
			return 0;
		}
	}
...
void mtk_snand_dev_unlock_all_blocks(struct mtk_snfc *snfc)
{
	...
	if (lock & SNAND_BLOCK_LOCK_BITS)
		pr_info("[SNF] Unlock all blocks failed!\n\r");
}
...
void mtk_snand_dev_ecc_control(struct mtk_snfc *snfc)
{
	...
	if (otp != otp_new) {
		...
		otp  = (reg>>16)&0xFF;
		if (otp != otp_new)
			pr_info("spi nand ecc disable fail");
		else
			pr_info("spi nand ecc disable success");
	}
}
...
static bool mtk_snand_reset_con(struct mtk_snfc *snfc)
{
	...
	for (;;) {
		...
		timeout--;
		if (!timeout) {
			pr_info("NFI_MASTER_STA timeout!!\n");
			return 0;
		}
	}
					    ...
					    u8 auto_fmt, u8 ahb_mode)
{
	...
	if (rc) {
		dev_info(snfc->dev, "dma mapping error\n");
		return -EINVAL;
	}
#endif
...
static bool mtk_snand_dma_read_data(struct mtd_info *mtd, u8 *buf, u32 num_sec)
{
	...
	if (interrupt_en) {
		if (!wait_for_completion_timeout(&g_comp_AHB_Done, 20)) {
			pr_info("wait for dma completion timeout\n");
		...
		while (num_sec > ((snfi_readw(snfc, NFI_BYTELEN) & 0xf000)
			...
			timeout--;
			if (timeout == 0) {
				pr_info("AHB mode poll BYTELEN error\n");
				g_snand_dev_status = SNAND_IDLE;
				g_running_dma = 0;
		...
		while (num_sec > ((snfi_readw(snfc, NFI_BYTELEN) & 0xf000)
			...
			timeout--;
			if (timeout == 0) {
				pr_info("PIO mode poll BYTELEN error\n");
				mtk_snand_dump_reg(snfc);
				g_snand_dev_status = SNAND_IDLE;
				g_running_dma = 0;
				     ...
				     u32 u4Size, u8 full)
{
	...
	if (i4Interrupt) {
		if (!wait_for_completion_timeout(&g_comp_AHB_Done, 20)) {
			pr_info("wait for completion timeout @[%s]: %d\n",
				__func__, __LINE__);
			mtk_snand_dump_reg(snfc);
			g_running_dma = 0;
			return 0;
...
mtk_nand_exec_read_page_retry:
	...
	if (bRet == 0) {
		if (retry < 0) {
			retry++;
			pr_info("[%s] read retrying (the %d time)\n",
				__func__, retry);
				...
				int oob_required, int page, int cached, int raw)
{
	...
	if (mtk_nand_exec_write_page(mtd, page, mtd->writesize,
				     (u8 *) buf, local_oob_buf, 0)) {
		pr_info("write fail at: 0x%x, page: 0x%x\n",
				     block, page_in_block);
	}
...
static void mtk_snand_select_chip(struct mtd_info *mtd, int chip)
{
	...
	if (!snfc) {
		pr_info("failed to allocate device structure.\n");
		return;
	}
	...
	if (chip == -1 && g_bInitDone == 0) {
		...
		if (spare_per_sector >= 28) {
			...
			pr_info("[NAND]: NFI not support oobsize: %x\n",
				spare_per_sector);
		}
				     ...
				     int oob_required, int page)
{
	...
	memset(local_oob_buf, 0xFF, mtd->oobsize);
	if (u4ColAddr == 0) {
		mtk_nand_exec_read_page(mtd, page, u4PageSize, buf,
					local_oob_buf);
...
int mtk_nand_erase_hw(struct mtd_info *mtd, int page)
{
	...
	if (ret & NAND_STATUS_FAIL)
		pr_info("[K-SNAND][%s] Erase blk %d failed!\n", __func__,
			page / 64);
...
static int mtk_nand_erase(struct mtd_info *mtd, int page)
{
	...
	if (status & NAND_STATUS_FAIL)
		pr_info("[%s] Erase fail at block: %d\n", __func__, block);
...
int mtk_nand_block_markbad_hw(struct mtd_info *mtd, loff_t offset)
{
	...
	u32 u4SecNum = u4PageSize >> 9;
	unsigned int bm_offset = (u4SecNum - 1) * OOB_AVAI_PER_SECTOR;
	int ret;
	u8 pFDMBuf[64];

	memset(pFDMBuf, 0xFF, 64);
	memset(local_page_buf, 0xFF, mtd->writesize);
	/* Bad mark at the first byte of the last sector */
	pFDMBuf[bm_offset] = 0;

	ret = mtk_nand_exec_write_page(mtd, page, u4PageSize, local_page_buf, pFDMBuf, 0);
	return ret;
}
...
static int mtk_snand_block_markbad(struct mtd_info *mtd, loff_t offset)
{
	...
	return  mtk_nand_block_markbad_hw(mtd, block
...
static int mtk_snand_read_oob(struct mtd_info *mtd, struct nand_chip *chip,
			      int page)
{
	return  mtk_snand_read_oob_hw(mtd, chip, page);
}
...
int mtk_nand_block_bad_hw(struct mtd_info *mtd, loff_t ofs)
{
	...
	unsigned int u4PageSize = 1 << chip->page_shift;
	u32 u4SecNum = u4PageSize >> 9;
	unsigned int offset = (u4SecNum - 1) * OOB_AVAI_PER_SECTOR;
	...
	if (mtk_snand_read_oob_raw(mtd, oob_buf, page_addr, u4SecNum * OOB_AVAI_PER_SECTOR)
				   == 0) {
		pr_info("mtk_snand_read_oob_raw return error\n");
		return 1;
	}

	if (oob_buf[offset] != 0xff) {
		pr_info("Bad block at 0x%x (blk:%d), Badmark is 0x%x\n",
...
static int mtk_snand_block_bad(struct mtd_info *mtd, loff_t ofs)
{
	...
	ret = mtk_nand_block_bad_hw(mtd, block
				    << chip->phys_erase_shift);

	if (ret) {
		pr_info("bad block: 0x%x\n", block);
		ret = 1;
	}
...
static bool mtk_snand_init_hw(struct mtk_snfc *snfc,
			      struct mtk_snand_host *host)
{
	...
	if (!mtk_snand_reset_con(snfc)) {
		dev_info(dev, "mtk snand reset controller Fail\n");
		return 0;
	}
	...
	mdelay(10);
	if (!mtk_snand_reset_dev(snfc)) {
		dev_info(dev, "mtk snand reset device Fail\n");
		return 0;
	}
...
void mtk_nand_test_kernel(struct nand_chip *nand_chip, struct mtd_info *mtd)
{
	...
	mtk_nand_exec_read_page(mtd, PAGE_IDX, PAGE_SIZE_TEST, pDateBuf_r,
				pFdmBuf);
	pr_info("read data after erase: 0x%x\n", pDateBuf_r[0]);

	memset(pDateBuf_w, 0xBC, PAGE_SIZE_TEST);

	if (nand_chip->write_page(mtd, nand_chip, 0, PAGE_SIZE_TEST,
				  pDateBuf_w, 0, PAGE_IDX, 0, 0))
		pr_info("[Test]Write page fail!\n");
	else
		pr_info("[Test]Write page Success!\n");

	memset(pDateBuf_r, 0xFE, PAGE_SIZE_TEST);
	mtk_nand_exec_read_page(mtd, PAGE_IDX, PAGE_SIZE_TEST, pDateBuf_r,
				pFdmBuf);

	pr_info("read data after write: 0x%x\n", pDateBuf_r[0]);

	if (memcmp(pDateBuf_w, pDateBuf_r, PAGE_SIZE_TEST))
		pr_info("[Test]Compare data fail!\n");
	else
		pr_info("[Test]Compare data success!\n");
}
#endif
...
static int mtk_snfc_enable_clk(struct device *dev, struct mtk_snfc_clk *clk)
{
	...
	if (ret) {
		dev_info(dev, "failed to enable nfi clk\n");
		return ret;
	}

	ret = clk_prepare_enable(clk->pad_clk);
	if (ret) {
		dev_info(dev, "failed to enable pad clk\n");
	...
	if (ret) {
		dev_info(dev, "failed to enable ecc clk\n");
		clk_disable_unprepare(clk->nfi_clk);
...
static int mtk_snand_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_info(dev, "failed to set dma mask\n");
		goto out;
	}
	...
	for (i = 0; i < nsels; i++) {
		...
		if (ret) {
			dev_info(dev, "reg property failure : %d\n", ret);
			return ret;
		}
		chip->sels[i] = tmp;
	}
	...
	if (!mtk_snand_init_hw(snfc, host)) {
		dev_info(dev, "mtk snand init hw Fail\n");
		goto out;
	}
	/* Select the device */
	...
	if (!mtk_snand_get_device_info(id, &devinfo)) {
		dev_info(dev, "Not Support this Device! \r\n");
		goto out;
	}
	...
	if (!np) {
		dev_info(dev, "no snand device to configure\n");
		return -ENODEV;
	}
	...
	if (err) {
		dev_info(dev, "[mtk_snand] mtd parse partition error!\n");
		nand_release(mtd);
		return err;
	}
	...
	/* Successfully!! */
	if (!err) {
		dev_info(dev, "[mtk_snand] probe successfully!\n");
