...
static void mtk_pcie_enable_port(struct mtk_pcie_port *port)
{
	...
	/* Assert phy to non-reset state, which active low */
			       ...
			       int slot)
{
	...
	snprintf(name, sizeof(name), "phy-rst%d", slot);
	port->phy_reset = devm_reset_control_get_optional(dev, name);
...
static const struct of_device_id mtk_pcie_ids[] = {
	...
	{ .compatible = "mediatek,leopard-pcie", .data = &mtk_pcie_soc_v2 },
	{},
};
...
DECLARE_PCI_FIXUP_HEADER(0x14c3, 0x5396, mtk_fixup_bar0);
