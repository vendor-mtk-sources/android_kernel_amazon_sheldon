...
int32_t cli_register(char *module_name, CLI_EXEC_T item[])
{
	...
	if (!global_cli_is_ready || module_name == NULL) {
		CLI_ERR("cli ready:%d module_name:%s\n",
			global_cli_is_ready, module_name);
		return 0;
	}
	...
	for (i = 0; item[i].function != NULL; i++) {
		...
		if(pItem == NULL) {
			CLI_ERR("alloc pItem fail\n");
			return 0;
		}

		if (snprintf(pItem->module_name, sizeof(pItem->module_name), "%s", module_name) < 0) {
			CLI_ERR("register module:%s fail\n", module_name);
			return 0;
		}
...
enum CLI_STATUS cli_core_fill_items(struct cli_items_struct *pItem)
{
	...
	list_for_each_entry(pTempItem, &gCliInfo.gCliListHead, list) {
		...
		size = snprintf(pUserItem->name, sizeof(pUserItem->name), "%s", pTempItem->module_case.name);
		if (size < 0) {
			CLI_ERR("copy string fail:%d\n", __LINE__);
			return CLI_STATUS_OK;
		}
		size = snprintf(pUserItem->module_name, sizeof(pUserItem->module_name), "%s", pTempItem->module_name);
		if (size < 0) {
			CLI_ERR("copy string fail:%d\n", __LINE__);
			return CLI_STATUS_OK;
		}
		size = snprintf(pUserItem->detail, sizeof(pUserItem->detail), "%s", pTempItem->module_case.detail);
		if (size < 0) {
			CLI_ERR("copy string fail:%d\n", __LINE__);
			return CLI_STATUS_OK;
		}
...
static long cli_ioctl(struct file *pFile, unsigned int code, unsigned long param)
{
	...
	struct cli_items_struct *pItems = NULL;
