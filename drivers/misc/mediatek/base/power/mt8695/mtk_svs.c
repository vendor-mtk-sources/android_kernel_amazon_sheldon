...
#define SVS_COMPATIBLE_NODE	"mediatek,mt8695-svs"
...
#define FUNC_ENTER(lv)    do { if ((lv) & func_lv_mask) pr_debug(">> %s()\n", __func__); } while (0)
#define FUNC_EXIT(lv)     do { if ((lv) & func_lv_mask) pr_debug("<< %s():%d\n", __func__, __LINE__); } while (0)
#else
...
#else
	ts_info.ts_MTS = 0x2CF;
	ts_info.ts_BTS = 0x80E;
#endif
...
static void base_ops_set_phase(struct svs_det *det, svs_phase phase)
{
	...
	svs_write(SVS_SVSCHKSHIFT, ((svs_read(SVS_SVSCHKSHIFT) & ~0xff) | 0x77));
	...
	case SVS_PHASE_INIT02:
		...
		svs_write(SVS_INIT2VALS,
				...
				((svs_devinfo.FT_PGM <= 2) ? 0 : det->DCVOFFSETIN & 0xffff));

		if (svs_log_en) {
			pr_notice("%s: svs_devinfo.FT_PGM:0x%08x, SVS_INIT2VALS:0x%08x\n",
				   __func__, svs_devinfo.FT_PGM, svs_read(SVS_INIT2VALS));
		}
...
static int unlimit_mcusys_env(struct svs_det *det)
{
	...
	clk_disable_unprepare(count_clk);
...
static void svs_set_svs_volt(struct svs_det *det)
{
	...
	if (svs_log_en)
		pr_err("svs_set_svs_volt(): cur_temp = %d\n", cur_temp);
...
static inline void handle_mon_mode_isr(struct svs_det *det)
{
	...
	if ((temp > 0x4b) && (temp < 0xd3)) {
		pr_notice("thermal sensor init has not been completed.(temp = 0x%08X)\n", temp);
		goto out;
	}
...
static int svs_probe(struct platform_device *pdev)
{
	...
	count_clk = devm_clk_get(&pdev->dev, "count_clk");
	if (IS_ERR(count_clk)) {
		dev_dbg(&pdev->dev, "failed to get clock: %ld\n", PTR_ERR(count_clk));
		return PTR_ERR(count_clk);
	}
	...
	ret = clk_prepare_enable(count_clk);
	if (ret)
		goto err_out1;
