...
static int hdmi_cec_txmsgq_dump(char *str)
{
	...
	while (!(tx_read_idx == tx_write_idx)) {
		...
		if (txmsg->size > 0)
			len += sprintf(str + strlen(str), "txmsgq[%02d]=0x%02x", (i - 1),
			(txmsg->blocks.header.initiator << 4) | txmsg->blocks.header.destination);
		if (txmsg->size > 1)
			len += sprintf(str + strlen(str), ",0x%02x", txmsg->blocks.opcode);
		for (j = 2; (j < txmsg->size) && (j <= 16); j++)
			len += sprintf(str + strlen(str), ",0x%02x", txmsg->blocks.operand[j - 2]);

		len += sprintf(str + strlen(str), "\n");
		i++;
	}
...
static int hdmi_cec_rxmsgq_dump(char *str)
{
	...
	while (!(rx_read_idx == rx_write_idx)) {
		...
		if (rxmsg->size > 0)
			len += sprintf(str + strlen(str), "rxmsgq[%02d]=0x%02x", (i - 1),
			(rxmsg->blocks.header.initiator << 4) | rxmsg->blocks.header.destination);
		if (rxmsg->size > 1)
			len += sprintf(str + strlen(str), ",0x%02x", rxmsg->blocks.opcode);
		for (j = 2; (j < rxmsg->size) && (j <= 16); j++)
			len += sprintf(str + strlen(str), ",0x%02x", rxmsg->blocks.operand[j - 2]);
...
static int hdmi_cec_level_dump(char *str)
{
	...
	len = sprintf(str + strlen(str), "cec level=%d\n", mtkcec_hw_input(global_cec));
...
static int hdmi_cec_args_dump(char *str)
{
	...
	len = sprintf(str + strlen(str), "pa[0-1]=0x%04x\n", *cec->kh.saved_arg.pa);
	len += sprintf(str + strlen(str), "version=0x%02x\n", cec->kh.saved_arg.version);
	len += sprintf(str + strlen(str), "device_type=0x%02x\n", cec->kh.saved_arg.device_type);
	len += sprintf(str + strlen(str), "vendor_id[0-2]=0x%02x,0x%02x,0x%02x\n",
		cec->kh.saved_arg.vendor_id[0], cec->kh.saved_arg.vendor_id[1], cec->kh.saved_arg.vendor_id[2]);
	len += sprintf(str + strlen(str), "osd_name=%s\n", name);
...
static void CEC_KHandle_BuildReplyMsg(struct mtk_cec *cec)
{
	...
	case OPCODE_REQUEST_ACTIVE_SOURCE:
		...
		break;
	case OPCODE_GIVE_OSD_NAME:
		cec->kh.txing_frame.txtag = (unsigned int)sequence;
...
void hdmi_CECMWSend(CEC_SEND_MSG *msg)
{
	...
	/*cec->txmsg_usr = msg;*/
	memcpy((void *)&cec->txmsg_usr, (void *)msg, sizeof(CEC_SEND_MSG));
	...
	msg->b_enqueue_ok = cec->txmsg_usr.b_enqueue_ok;
	msg->result.e_ack_cond = cec->txmsg_usr.result.e_ack_cond;
	msg->result.pv_tag = cec->txmsg_usr.result.pv_tag;
	msg->pv_tag = cec->txmsg_usr.pv_tag;
}
