...
bool output_format_already_change = false;
		...
		if (((layer_info->timeline_idx - 1) == buf->current_fence_index)
			...
			if (layer_info->timeline != NULL) {
				...
				if (print_video_fence_history) {
					...
					DISP_LOG_N("fence%d time 0x%llx, wait 0x%llx pts %lld, timer %ld\n",
	...
	enum HDR10_TYPE_ENUM buffer_type)
{
	...
	*/
	...
	if (vdp_check_2160p_timing(disp_common_info.resolution->res_mode)) {
		...
		if ((info.hd_scl_on) && (dsd_en == 0) && (vdp_id == DISP_FMT_MAIN)) {
	...
	const struct disp_hw_resolution *resolution)
{
	...
	if ((dolby_out_format_res_change != dolby_out_format) || (output_format_already_change)) {
		if (!output_format_already_change)
	...
	if ((old_resolution != resolution->res_mode)
		...
		output_format_already_change = false;
	}
	...
	struct video_buffer_info *buf)
{
	...
	bool is_2160p60_out = vdp_check_2160p60_timing(resolution->res_mode);
	struct disp_hw *vdp_drv = disp_vdp_get_drv();
	enum dovi_signal_format_t original_dolby_out_format = dolby_out_format;
	...
	if (original_dolby_out_format != dolby_out_format) {
		DISP_LOG_N("%s, original_dolby_out_format=%d, dolby_out_format=%d\n",
			__func__, original_dolby_out_format, dolby_out_format);
		output_format_already_change = true;
		if (original_dolby_out_format == DOVI_FORMAT_DOVI)
			vDolbyHdrEnable(false);
		else if (original_dolby_out_format == DOVI_FORMAT_HDR10) {
			vHdrEnable(false);
			vdp_set_HDMI_BT2020_signal(false);
		} else if (original_dolby_out_format == DOVI_FORMAT_DOVI_LOW_LATENCY)
			vLowLatencyDolbyVisionEnable(false);
	}
	vdp_dovi_info.out_format = dolby_out_format;
	if (osd_enable)
	...
	struct video_buffer_info *buf, int dsd_en)
{
	...
	#ifdef CONFIG_MTK_DOVI_NO_SUPPORT
	return;
	#endif
	...
	uint32_t ctl_info)
{
	...
	uint8_t application_identifier = 0;
	uint8_t application_version = 0;
	static uint8_t application_version_pre = 0;
	uint8_t num_windows = 0;
	uint8_t num_idx = 0;
	uint8_t idx = 0;
	uint32_t sel_tsdmaxl = 0;
	...
	if (application_version_pre != application_version) {
		application_version_pre = application_version;
		DISP_LOG_I("[VDP]HDR10PLus, application version of the first frame or app version changed, application_version:%d\n",
			application_version);
	}
	vdp_printf(VDP_HDR10PLUS_LOG,
...
static int vdp_routine(void *data)
{
	...
	/* just for dolby idk. Guarantee osd not to cover video even if video is full screen. */
	#if 0
	int params_count = 7;
	const char *vdp_show[7] = {"vdp.cli", "0", "0", "0", "1920", "1080", "1920"};
	#endif
	...
	while (1) {
		...
		for (i = thread_id; i < (thread_id + video_stream_number); i++) {
			...
			if (dovi_idk_dump && (i == 0)) {
				/* just for dolby idk. Guarantee osd not to cover video even if video is full screen. */
				#if 0
				_vdp_cli_debug_set_disp_area(params_count, vdp_show);
				#endif
			...
			*/
			...
			/* just for dolby idk => run this code */
			#if 0
