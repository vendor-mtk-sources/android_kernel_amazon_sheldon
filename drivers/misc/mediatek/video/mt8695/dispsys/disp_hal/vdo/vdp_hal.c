...
{
	union vdo_hal_union *p_vdp_sw_reg = NULL;
	uint64_t *p_vdp_reg_mode = NULL;

	union vdo_hal_ufo_union *p_ufo_sw_reg = NULL;
	uint64_t *p_ufo_reg_mode = NULL;
...
void vdp_hal_set_src_size(uint8_t vdp_id, uint32_t width, uint32_t height)
{
	...
	union vdo_hal_union *p_vdp_sw_reg = NULL;
	union vdo_hal_ufo_union *p_ufo_sw_reg = NULL;
	union vdo_hal_scl_union *p_vdp_scl_sw_reg = NULL;

	uint64_t *p_vdp_reg_mode = NULL;
	uint64_t *p_ufo_reg_mode = NULL;
	uint64_t *p_vdp_scl_reg_mode = NULL;
...
void vdp_hal_set_src_color_format(uint8_t vdp_id, struct vdp_hal_color_format *p_clr_fmt)
{
	...
	p_vdp_sw_reg->field.YUV422 = p_clr_fmt->is_yuv422;
	...
	REG_SET(*p_ufo_reg_mode, REG_MASK(0x18 / 4) |
		REG_MASK(0x1C / 4));
	REG_SET(*p_vdp_reg_mode, REG_MASK(0x30 / 4));
}
